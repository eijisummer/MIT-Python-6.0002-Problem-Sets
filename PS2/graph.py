...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return f'{self.src}->{self.dest} ({str(self.total_distance)}, {str(self.outdoor_distance)})'

# example = WeightedEdge('x', 'y', 3, 2)
# print('Example', example)

class Digraph(object):
    """Represents a directed graph of Node and Edge objects"""
    def __init__(self):
        self.nodes = set([]) #creates a set.  Example: {1,2,... }.  Sets are mutable
...
class TestGraph(unittest.TestCase):
    ...
    def test_graph_str(self):
        expected = "a->b (15, 10)\na->c (14, 6)\nb->c (3, 1)" #Corresponds to def __str__() in Digraph()
