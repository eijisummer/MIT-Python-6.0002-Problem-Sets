...
# Name:  Eijiro Asada
# Collaborators (discussion): Nobody
# Time:A lot of time
...
class Position(object):
    ...
    def get_new_position(self, angle, speed):
        ...
        """
        old_x, old_y = self.get_x(), self.get_y() #eg. old_x = a.get_x() = 3
        ...
        return Position(new_x, new_y) # e.g. get_new_position(2, 205) = new position
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.width = width
        self.height = height
        self.dirt_amount = dirt_amount #dirt amout per tile
        
        self.tiles = {}
        
        if self.width>0 and self.height>0 and self.dirt_amount >=0:
...
class Robot(object):
    ...
    def set_robot_direction(self, direction):
        ...
        if direction<0 or direction>360:
            raise ValueError('Direction value must be between 0 and 360')
        else:
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        pos_x = random.uniform(0, self.width)
        pos_y = random.uniform(0,self.height)
        random_position = Position(pos_x, pos_y)
...
class FaultyRobot(Robot):
    ...
    def gets_faulty(self):
        ...
        """
        return random.random() < FaultyRobot.p #random.random() returns a random 
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        for i in range(num_robots):
            ...
            list_robots.append(robot)
        
        time_steps = 0        
        
        min_coverage_tiles = room.get_num_tiles()*min_coverage
        cleaned_tiles = 0
        while cleaned_tiles < min_coverage_tiles:
            for j in range(len(list_robots)):
                list_robots[j].update_position_and_clean()         
            time_steps += 1
            cleaned_tiles = room.get_num_cleaned_tiles()
        
        list_time_steps.append(time_steps)
    
    return sum(list_time_steps)/num_trials
